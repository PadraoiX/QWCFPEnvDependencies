(function(a) {
	if (a.PrimeFaces) {
		b.debug("PrimeFaces already loaded, ignoring duplicate execution.");
		return
	}
	var b = {
		...
		isIE : function(c) {
			return (c === undefined) ? this.browser.msie
					: (this.browser.msie && parseInt(this.browser.version, 10) === c)
		},
		isIOS : function() {
			return /iPhone|iPad|iPod/i.test(a.navigator.userAgent)
		},
		info : function(c) {
			if (this.logger) {
				this.logger.info(c)
		...
		changeTheme : function(g) {
			if (g && g !== "") {
				var h = $('link[href*="javax.faces.resource/theme.css"]'), f = h
						.attr("href"), e = f.split("&")[0], d = e.split("ln=")[1], c = f
		...
		createWidget : function(c, i, e, g) {
			if (b.widget[c]) {
				if (b.widgets[i]) {
					b.widgets[i].refresh(e)
				} else {
					b.widgets[i] = new b.widget[c](e);
					if (b.settings.legacyWidgetNamespace) {
						a[i] = b.widgets[i]
					}
				}
			} else {
				var f = b.getFacesResource(g + "/" + g + ".js", "primefaces");
				var h = b.getFacesResource(g + "/" + g + ".css", "primefaces");
				var d = '<link type="text/css" rel="stylesheet" href="' + h
						+ '" />';
				$("head").append(d);
		...
		getFacesResource : function(f, e, c) {
			...
			if (c) {
				var g = new RegExp("[?&]v=([^&]*)").exec(d)[1];
				d = d.replace("v=" + g, "v=" + c)
			}
			return a.location.protocol + "//" + a.location.host + d
		},
		...
		resolveUserAgent : function() {
			if ($.browser) {
				...
				var c, f;
				jQuery.uaMatch = function(j) {
					j = j.toLowerCase();
					var i = /(opr)[\/]([\w.]+)/.exec(j)
							|| /(chrome)[ \/]([\w.]+)/.exec(j)
							|| /(version)[ \/]([\w.]+).*(safari)[ \/]([\w.]+)/
									.exec(j) || /(webkit)[ \/]([\w.]+)/.exec(j)
							|| /(opera)(?:.*version|)[ \/]([\w.]+)/.exec(j)
							|| /(msie) ([\w.]+)/.exec(j)
							|| j.indexOf("trident") >= 0
							&& /(rv)(?::| )([\w.]+)/.exec(j)
							|| j.indexOf("compatible") < 0
							&& /(mozilla)(?:.*? rv:([\w.]+)|)/.exec(j) || [];
					var h = /(ipad)/.exec(j) || /(iphone)/.exec(j)
							|| /(android)/.exec(j) || /(windows phone)/.exec(j)
							|| /(win)/.exec(j) || /(mac)/.exec(j)
							|| /(linux)/.exec(j) || /(cros)/i.exec(j) || [];
					return {
						browser : i[3] || i[1] || "",
						version : i[2] || "0",
						platform : h[0] || ""
				...
				c = jQuery.uaMatch(a.navigator.userAgent);
				f = {};
				if (c.browser) {
					f[c.browser] = true;
					f.version = c.version;
					f.versionNumber = parseInt(c.version)
				}
				if (c.platform) {
					f[c.platform] = true
				}
				if (f.android || f.ipad || f.iphone || f["windows phone"]) {
					f.mobile = true
				}
				if (f.cros || f.mac || f.linux || f.win) {
					f.desktop = true
				}
				if (f.chrome || f.opr || f.safari) {
					f.webkit = true
				}
				if (f.rv) {
					var g = "msie";
...
PrimeFaces.ajax = {
	...
	Utils : {
		...
		updateFormStateInput : function(c, g, h) {
			var e = $.trim(g);
			var b = null;
			if (h && h.pfSettings && h.pfSettings.portletForms) {
				b = $(h.pfSettings.portletForms)
			} else {
				b = $("form")
			}
			for (var d = 0; d < b.length; d++) {
				...
				if (f.attr("method") === "post") {
					var a = f.children("input[name='" + c + "']");
					if (a.length > 0) {
						a.val(e)
					} else {
						f.append('<input type="hidden" name="' + c
	...
	Request : {
		...
		send : function(e) {
			...
			if (e.formId) {
				b = $(PrimeFaces.escapeClientId(e.formId))
			} else {
				b = $(PrimeFaces.escapeClientId(f)).parents("form:first");
				if (b.length === 0) {
					b = $("form").eq(0)
				}
			}
			...
			PrimeFaces.debug("URL to post " + v + ".");
			g.push({
			...
			if (e.partialSubmit && a.indexOf("@all") === -1) {
				...
				if (!o) {
					g.push({
						...
						value : b.children(
								"input[name='" + PrimeFaces.VIEW_STATE + "']")
								.val()
					});
					var k = b.children("input[name='"
							+ PrimeFaces.CLIENT_WINDOW + "']");
					if (k.length > 0) {
						g.push({
							name : PrimeFaces.CLIENT_WINDOW,
							value : k.val()
						})
					}
					var r = b.children("input[name='dsPostWindowId']");
					if (r.length > 0) {
						g.push({
							name : "dsPostWindowId",
							value : r.val()
					...
					r = b.children("input[name='dspwid']");
					if (r.length > 0) {
						g.push({
							name : "dspwid",
							value : r.val()
	...
	Response : {
		...
		destroyDetachedWidgets : function() {
			for (var a = 0; a < PrimeFaces.detachedWidgets.length; a++) {
				...
				if (b) {
					if (b.isDetached()) {
						PrimeFaces.removeDeferredRenders(b.id);
	...
	ResponseProcessor : {
		...
		doExtension : function(b, c) {
			if (c) {
				if (b.getAttribute("ln") === "primefaces"
						&& b.getAttribute("type") === "args") {
					var a = b.textContent || b.innerText || b.text;
					c.pfArgs = $.parseJSON(a)
				}
			}
		},
						...
						if (m.indexOf("@widgetVar(") == 0) {
							...
							if (m.indexOf("@(") == 0) {
								...
								for (var e = 0; e < b.length; e++) {
									var f = $(b[e]);
									var a = f.data(PrimeFaces.CLIENT_ID_DATA)
											|| f.attr("id");
...
PrimeFaces.dialog.DialogHandler = {
	openDialog : function(e) {
		...
		var h = e.sourceComponentId.replace(/:/g, "_") + "_dlgwidget", f = $(
				'<div id="'
						+ d
						+ '" class="ui-dialog ui-widget ui-widget-content ui-corner-all ui-shadow ui-overlay-hidden" data-pfdlgcid="'
		...
		if (e.options.closable !== false) {
			f
					...
					.append(
							'<a class="ui-dialog-titlebar-icon ui-dialog-titlebar-close ui-corner-all" href="#" role="button"><span class="ui-icon ui-icon-closethick"></span></a>')
		}
		f
				.append('<div class="ui-dialog-content ui-widget-content" style="height: auto;"><iframe style="border:0 none" frameborder="0"/></div>');
		f.appendTo(document.body);
		var c = f.find("iframe"), g = e.url.indexOf("?") === -1 ? "?" : "&", b = e.url
				+ g + "pfdlgcid=" + e.pfdlgcid, a = e.options.contentWidth || 640;
		c.width(a);
		c
				.on(
						"load",
						function() {
								...
								PrimeFaces
										.cw(
												...
												h,
												{
													...
													onHide : function() {
														var m = this, l = this.content
																.children("iframe");
														if (l.get(0).contentWindow.PrimeFaces) {
															this.destroyIntervalId = setInterval(
																	function() {
																		if (l
																				.get(0).contentWindow.PrimeFaces.ajax.Queue
																				.isEmpty()) {
																			clearInterval(m.destroyIntervalId);
																			l
																					.attr(
																							"src",
																							"about:blank");
							...
							if (k.length > 0) {
								PF(h).titlebar.children("span.ui-dialog-title")
										.html(k.text())
							}
							var i = e.options.contentHeight
									|| j.get(0).contentWindow.document.body.scrollHeight
									+ 5;
							j.height(i);
							PF(h).show();
...
PrimeFaces.widget.AccordionPanel = PrimeFaces.widget.BaseWidget.extend({
	...
	unselect : function(b) {
		...
		c.attr("aria-expanded", false).children(".ui-icon").removeClass(
				this.cfg.expandedIcon).addClass(this.cfg.collapsedIcon);
	...
	show : function(c) {
		...
		if (!this.cfg.multiple) {
			...
			d.children(".ui-icon").removeClass(this.cfg.expandedIcon).addClass(
					this.cfg.collapsedIcon);
			d.attr("aria-expanded", false).removeClass(
					"ui-state-active ui-corner-top").addClass("ui-corner-all")
					.next().attr("aria-hidden", true).slideUp()
		}
		var a = c.prev();
		a.attr("aria-expanded", true).addClass("ui-state-active ui-corner-top")
				.removeClass("ui-state-hover ui-corner-all").children(
						".ui-icon").removeClass(this.cfg.collapsedIcon)
...
PrimeFaces.widget.AjaxStatus = PrimeFaces.widget.BaseWidget.extend({
	...
	trigger : function(a) {
		...
		this.jq.children().hide().filter(this.jqId + "_" + a).show()
	},
	bindToStandard : function() {
		if (window.jsf && window.jsf.ajax) {
			...
			jsf.ajax.addOnEvent(function(b) {
				if (b.status === "begin") {
					a.trigger("pfAjaxStart")
				} else {
					if (b.status === "complete") {
						a.trigger("pfAjaxSuccess")
					} else {
						if (b.status === "success") {
							a.trigger("pfAjaxComplete")
						}
					}
				}
			});
			jsf.ajax.addOnError(function(b) {
				a.trigger("pfAjaxError")
			})
		}
	}
});
PrimeFaces.widget.AutoComplete = PrimeFaces.widget.BaseWidget
		.extend({
			...
			setupMultipleMode : function() {
				...
				this.multiItemContainer.off("click", a).on("click", a, null,
						function(c) {
			...
			removeItem : function(b, a) {
				...
				this.hinput.children("option").filter('[value="' + d + '"]')
						.remove();
			...
			groupItems : function() {
				...
				if (this.items.length) {
					...
					this.items.each(function(b) {
						var c = a.items.eq(b), d = a.items.eq(b).data(
								"item-group");
						if (a.currentGroup !== d) {
							a.currentGroup = d;
							c.before(a.getGroupItem(d, a.itemContainer))
						}
					})
				}
			},
			getGroupItem : function(b, a) {
				if (a.is(".ui-autocomplete-table")) {
					if (!this.colspan) {
						this.colspan = this.items.eq(0).children("td").length
					}
					return '<tr class="ui-autocomplete-group ui-widget-header"><td colspan="'
							+ this.colspan + '">' + b + "</td></tr>"
				} else {
					return '<li class="ui-autocomplete-group ui-autocomplete-list-item ui-widget-header">'
							+ b + "</li>"
				}
			},
...
PrimeFaces.widget.Carousel = PrimeFaces.widget.BaseWidget
		.extend({
			init : function(b) {
				this._super(b);
				...
				this.prevButton = this.header
						.children(".ui-carousel-prev-button");
				this.nextButton = this.header
						.children(".ui-carousel-next-button");
				...
				this.cfg.pageCount = Math.ceil(this.items.length
						/ this.cfg.numVisible);
				...
				if (this.items.length) {
					...
					this.cfg.itemOuterWidth = c.innerWidth()
							+ parseInt(this.getProperty(a, "margin-Left"))
							+ parseInt(this.getProperty(a, "margin-Right"))
							+ ((parseInt(this.getProperty(a,
									"border-Left-Width")) + parseInt(this
									.getProperty(a, "border-Right-Width"))));
					this.cfg.itemOuterHeight = c.innerHeight()
							+ Math.max(parseInt(this.getProperty(a,
									"margin-Top")), parseInt(this.getProperty(
									a, "margin-Bottom")))
							+ ((parseInt(this
									.getProperty(a, "border-Top-Width")) + parseInt(this
									.getProperty(a, "border-Bottom-Width"))));
					if (this.cfg.vertical) {
						this.viewport.width(this.cfg.itemOuterWidth);
						this.viewport.height(this.cfg.numVisible
								* this.cfg.itemOuterHeight)
					} else {
						this.viewport.width(this.cfg.numVisible
								* this.cfg.itemOuterWidth);
			...
			getProperty : function(a, b) {
				return $.browser.msie ? a.currentStyle.getAttribute(b.replace(
			...
			bindEvents : function() {
				...
				this.prevButton.click(function(b) {
					a.stopAutoplay();
					if (!a.prevButton.hasClass("ui-state-disabled")
							&& !a.animating) {
						a.prev()
					}
				});
				this.nextButton.click(function() {
					a.stopAutoplay();
					if (!a.nextButton.hasClass("ui-state-disabled")
							&& !a.animating) {
						a.next()
					}
				})
			...
			getPagePosition : function(a) {
				return -((a - 1)
						* (this.cfg.vertical ? this.cfg.itemOuterHeight
								: this.cfg.itemOuterWidth) * this.cfg.numVisible)
			},
			getItemPosition : function(a) {
				return -(a * (this.cfg.vertical ? this.cfg.itemOuterHeight
						: this.cfg.itemOuterWidth))
			},
			getPosition : function() {
				return parseInt(this.list.css(this.cfg.vertical ? "top"
						: "left"))
			},
			...
			slide : function(c) {
				var a = this, b = this.cfg.vertical ? {
					top : c
				} : {
					left : c
				};
			...
			setPage : function(a) {
				if (this.cfg.circular) {
					this.cfg.page = a > this.cfg.pageCount ? 1
							: a < 1 ? this.cfg.pageCount : a
				} else {
					this.cfg.page = a
				}
				this.checkButtons();
			...
			checkButtons : function() {
				this.pageLinks.filter(".ui-icon-radio-on").removeClass(
						"ui-icon-radio-on");
				this.pageLinks.eq(this.cfg.page - 1).addClass(
						"ui-icon-radio-on");
		...
		.extend({
			...
			render : function() {
				...
				for (var b = 0; b < this.columns.length; b++) {
					var c = this.columns.eq(b), d = c
							...
							: "ui-chkbox-icon ui-icon fa fa-fw fa-check-square-o";
					$(
			...
			check : function(b) {
				b.addClass("ui-state-active").removeClass("ui-state-hover")
						.children(".ui-chkbox-icon").addClass("fa fa-fw fa-check-square-o")
						.removeClass("ui-icon-blank");
				var a = $(
						document.getElementById(b.closest(
								"li.ui-columntoggler-item").data("column")))
						.index() + 1;
				this.thead.children("tr").find("th:nth-child(" + a + ")")
						.show();
			...
			uncheck : function(b) {
				b.removeClass("ui-state-active").children(".ui-chkbox-icon")
						.addClass("ui-icon-blank").removeClass("fa fa-fw fa-check-square-o");
				var a = $(
						document.getElementById(b.closest(
								"li.ui-columntoggler-item").data("column")))
						.index() + 1;
				...
				if (this.cfg.editMode === "row") {
					...
					this.tbody.off("click.datatable", a).on("click.datatable",
							a, null, function(f) {
								...
								if (d.hasClass("fa fa-fw fa-edit")) {
									c.switchToRowEdit(g);
									d.hide().siblings().show()
								} else {
									if (d.hasClass("fa fa-fw fa-check-square-o")) {
										c.saveRowEdit(g)
									} else {
				...
				if (this.cfg.closeOnEscape) {
					$(document).on(
							...
							function(d) {
								...
								if (d.which === c.ESCAPE
										&& a.jq.hasClass("ui-overlay-visible")
										&& b) {
									a.hide()
								}
							})
			...
			initPosition : function() {
				...
				if (/(center|left|top|right|bottom)/.test(this.cfg.position)) {
					...
					this.jq.position({
						...
						of : window,
						using : function(f) {
							var d = f.left < 0 ? 0 : f.left, e = f.top < 0 ? 0
									: f.top;
							$(this).css({
								left : d,
								top : e
					...
					var b = this.cfg.position.split(","), a = $.trim(b[0]), c = $
							.trim(b[1]);
					this.jq.offset({
						left : a,
						top : c
			...
			toggleMinimize : function() {
				...
				if (this.minimized) {
					...
					this.minimizeIcon.removeClass("ui-state-hover").children(
							".ui-icon").removeClass("fa fa-fw fa-plus-square-o").addClass(
							"ui-icon-minus");
	...
	showMessage : function(a) {
		...
		if (a.icon) {
			this.icon.removeClass().addClass(
					"ui-icon ui-confirm-dialog-severity " + a.icon)
		}
		this.show()
	}
});
...
PrimeFaces.widget.Fieldset = PrimeFaces.widget.BaseWidget.extend({
	...
	updateToggleState : function(a) {
		if (a) {
			this.toggler.removeClass("fa fa-fw fa-plus-square-o-square-o").addClass(
					"ui-icon-minusthick")
		} else {
			this.toggler.removeClass("ui-icon-minusthick").addClass(
					"fa fa-fw fa-plus-square-o-square-o")
		}
...
PrimeFaces.widget.InputTextarea = PrimeFaces.widget.BaseWidget
		.extend({
			...
			resize : function() {
				...
				var c = (d >= this.cfg.rowsDefault) ? (d + 1)
						: this.cfg.rowsDefault;
				this.jq.attr("rows", c)
			},
...
PrimeFaces.widget.SelectOneMenu = PrimeFaces.widget.DeferredWidget
		.extend({
			init : function(a) {
				...
				var d = this, c = this.options.filter(":selected");
				...
				if (this.cfg.editable) {
					...
					if (b === c.text()) {
						this.highlightItem(this.items.eq(c.index()))
					} else {
							...
							default:
								...
								if (c.length) {
									...
									if (a.panel.is(":hidden")) {
										...
										a.highlightItem(b);
										PrimeFaces.scrollInView(a.itemsWrapper,
												b)
									}
								}
			...
			filter : function(e) {
				...
				var g = this.cfg.caseSensitive ? $.trim(e) : $.trim(e)
						.toLowerCase();
				if (g === "") {
				...
				if (this.cfg.custom) {
					...
					for (var c = 0; c < this.outputs.length; c++) {
						this.labels = this.labels
								.add('label[for="'
										+ this.outputs.eq(c).parent()
												.attr("id") + '"]')
					}
					this.originalInputs = this.jq.find(":radio");
					for (var c = 0; c < this.originalInputs.length; c++) {
						var b = this.originalInputs.eq(c), d = $(PrimeFaces
								.escapeClientId(b.attr("id") + "_clone"));
						d.val(b.val());
						if (b.is(":checked")) {
							d.prop("checked", true).parent().next().addClass(
			...
			bindEvents : function() {
				...
				this.inputs
						...
						.on(
								...
								function(h) {
									var i = $(this), f = i.parent().next(), g = a.inputs
											.index(i), m = a.inputs.length, l = $.ui.keyCode, j = h.which;
									switch (j) {
									case l.UP:
									case l.LEFT:
										var c = (g === 0) ? a.inputs
												.eq((m - 1)) : a.inputs.eq(--g), k = c
												.parent().next();
										i.blur();
										a.unselect(f);
										a.select(k);
									...
									case l.DOWN:
									case l.RIGHT:
										var d = (g === (m - 1)) ? a.inputs
												.eq(0) : a.inputs.eq(++g), b = d
												.parent().next();
										i.blur();
										a.unselect(f);
										a.select(b);
		...
		if (!this.disabled) {
			...
			this.input.on("focus.selectBooleanCheckbox", function() {
				...
				if (d.which === c.SPACE) {
					...
					"change.selectBooleanCheckbox",
					function(c) {
						if (b.isChecked()) {
							b.box.addClass("ui-state-active").children(
									".ui-chkbox-icon").removeClass(
									"ui-icon-blank").addClass("fa fa-fw fa-check-square-o")
						} else {
							b.box.removeClass("ui-state-active").children(
									".ui-chkbox-icon")
...
PrimeFaces.widget.SelectListbox = PrimeFaces.widget.BaseWidget
		.extend({
			init : function(a) {
				...
				this.options = $(this.input).children("option");
				this.items = this.listElement
						.find(".ui-selectlistbox-item:not(.ui-state-disabled)");
				var b = this.options.filter(":selected:not(:disabled)");
				if (b.length) {
					PrimeFaces.scrollInView(this.jq, this.items.eq(b.eq(0)
							.index()))
				}
				this.bindEvents();
				this.input.data(PrimeFaces.CLIENT_ID_DATA, this.id)
...
PrimeFaces.widget.SelectCheckboxMenu = PrimeFaces.widget.BaseWidget
		.extend({
			...
			renderItems : function() {
				...
				this.inputs
						.each(function() {
							var h = $(this), g = h.next(), b = h
									.is(":disabled"), i = h.is(":checked"), e = h
									.attr("title"), c = "ui-chkbox-box ui-widget ui-corner-all ui-state-default", f = "ui-selectcheckboxmenu-item ui-selectcheckboxmenu-list-item ui-corner-all";
							if (b) {
								c += " ui-state-disabled"
							}
							if (i) {
							...
							var d = i ? "ui-chkbox-icon ui-icon fa fa-fw fa-check-square-o"
									: "ui-chkbox-icon ui-icon ui-icon-blank", f = i ? f
									+ " ui-selectcheckboxmenu-checked"
									: f + " ui-selectcheckboxmenu-unchecked";
							var j = $('<li class="' + f + '"></li>');
							j
									.append(
											'<div class="ui-chkbox ui-widget"><div class="ui-helper-hidden-accessible"><input type="checkbox" readonly="readonly"></input></div><div class="'
													+ c
													+ '"><span class="'
													+ d
													+ '"></span></div></div>')
									.append("<label></label>")
									.children("label").text(g.text());
							if (e) {
								j.attr("title", e)
							}
							j
			...
			check : function(c, b) {
				if (!c.hasClass("ui-state-disabled")) {
					c.addClass("ui-state-active").children(".ui-chkbox-icon")
							.removeClass("ui-icon-blank").addClass(
									"fa fa-fw fa-check-square-o");
					c.closest("li.ui-selectcheckboxmenu-item").removeClass(
							"ui-selectcheckboxmenu-unchecked").addClass(
							"ui-selectcheckboxmenu-checked");
					c.prev().children("input").prop("checked", true);
					if (b) {
				...
				if (!c.hasClass("ui-state-disabled")) {
					c.removeClass("ui-state-active")
							.children(".ui-chkbox-icon").addClass(
									"ui-icon-blank").removeClass(
									"fa fa-fw fa-check-square-o");
					c.closest("li.ui-selectcheckboxmenu-item").addClass(
							"ui-selectcheckboxmenu-unchecked").removeClass(
							"ui-selectcheckboxmenu-checked");
					c.prev().children("input").prop("checked", false);
					if (b) {
						var a = this.inputs.eq(c.parents("li:first").index());
			...
			bindCheckboxKeyEvents : function(a) {
				a.on("focus.selectCheckboxMenu", function(d) {
					var b = $(this), c = b.parent().next();
					if (b.prop("checked")) {
						c.removeClass("ui-state-active")
					}
...
PrimeFaces.widget.LightBox = PrimeFaces.widget.BaseWidget
		.extend({
			...
			bindCommonEvents : function() {
				var c = this, b = PrimeFaces.isIOS() ? "touchstart." + this.id
...
PrimeFaces.widget.Menubar = PrimeFaces.widget.TieredMenu
		.extend({
			showSubmenu : function(e, c) {
				var d = $(window), b = null, a = {
					"z-index" : ++PrimeFaces.zindex
				};
				if (e.parent().hasClass("ui-menu-child")) {
					a.left = e.outerWidth();
					a.top = 0;
				...
				this.keyboardTarget
						...
						.on(
								...
								function(j) {
									...
									var d = !h.closest("ul").hasClass(
											"ui-menu-child"), i = $.ui.keyCode;
									switch (j.which) {
									case i.LEFT:
										if (d) {
											var f = h
			...
			bindEvents : function() {
				...
				this.keyboardTarget
						...
						.on(
								"keydown.menu",
								function(g) {
									var d = a.menuitemLinks
											.filter(".ui-state-hover"), f = $.ui.keyCode;
									switch (g.which) {
									case f.UP:
										var c = d.parent().prevAll(
												".ui-menuitem:first");
										if (c.length) {
											d.removeClass("ui-state-hover");
			...
			bindDataTable : function() {
				...
				$(document)
						...
						.on(
								...
								null,
								function(h) {
									var f = PrimeFaces.widgets[c.cfg.targetWidgetVar];
...
PrimeFaces.widget.PanelMenu = PrimeFaces.widget.BaseWidget
		.extend({
			...
			restoreState : function() {
				var d = PrimeFaces.getCookie(this.stateKey);
				if (d) {
					this.collapseAll();
...
PrimeFaces.widget.Panel = PrimeFaces.widget.BaseWidget.extend({
	...
	expand : function() {
		this.toggleState(false, "fa fa-fw fa-plus-square-o-square-o", "ui-icon-minusthick");
	...
	collapse : function() {
		this.toggleState(true, "ui-icon-minusthick", "fa fa-fw fa-plus-square-o-square-o");
		if (this.cfg.toggleOrientation === "vertical") {
			this.slideUp()
		} else {
			if (this.cfg.toggleOrientation === "horizontal") {
...
PrimeFaces.widget.OrderList = PrimeFaces.widget.BaseWidget
		.extend({
			...
			moveUp : function() {
				var a = this;
				this.items.filter(".ui-state-highlight").each(
						function() {
							var b = $(this);
							if (!b.is(":first-child")) {
								b.hide(a.cfg.effect, {}, "fast", function() {
									b.insertBefore(b.prev()).show(a.cfg.effect,
											{}, "fast", function() {
												a.saveState();
												a.fireReorderEvent()
											})
			...
			moveTop : function() {
				...
				this.items.filter(".ui-state-highlight").each(
						function() {
							...
							if (!b.is(":first-child")) {
								b.hide(a.cfg.effect, {}, "fast", function() {
									b.prependTo(b.parent()).show(a.cfg.effect,
											{}, "fast", function() {
												a.saveState();
												a.fireReorderEvent()
											})
			...
			moveDown : function() {
				var a = this;
				$(this.items.filter(".ui-state-highlight").get().reverse())
						.each(
								function() {
									var b = $(this);
									if (!b.is(":last-child")) {
			...
			moveBottom : function() {
				...
				this.items.filter(".ui-state-highlight").each(
						function() {
							...
							if (!b.is(":last-child")) {
								b.hide(a.cfg.effect, {}, "fast", function() {
									b.appendTo(b.parent()).show(a.cfg.effect,
											{}, "fast", function() {
												a.saveState();
												a.fireReorderEvent()
											})
...
PrimeFaces.widget.Slider = PrimeFaces.widget.BaseWidget.extend({
	...
	onSlide : function(a, b) {
		...
		if (this.cfg.range) {
			...
			if (this.output) {
				this.output.html(this.cfg.displayTemplate.replace("{min}",
						b.values[0]).replace("{max}", b.values[1]))
			}
		} else {
			this.input.val(b.value);
			if (this.output) {
				this.output.html(this.cfg.displayTemplate.replace("{value}",
						b.value))
			}
		}
	},
...
PrimeFaces.widget.Spotlight = PrimeFaces.widget.BaseWidget
		.extend({
			...
			createMasks : function() {
				$(document.body)
						.append(
								'<div class="ui-widget-overlay ui-spotlight ui-spotlight-top ui-helper-hidden"></div><div class="ui-widget-overlay ui-spotlight ui-spotlight-bottom ui-helper-hidden"></div><div class="ui-widget-overlay ui-spotlight ui-spotlight-left ui-helper-hidden"></div><div class="ui-widget-overlay ui-spotlight ui-spotlight-right ui-helper-hidden"></div>')
			},
			show : function() {
				var h = $(document), c = $(document.body), a = this.target
						.offset(), e = c.children("div.ui-spotlight-top"), b = c
						.children("div.ui-spotlight-bottom"), i = c
						.children("div.ui-spotlight-left"), d = c
						.children("div.ui-spotlight-right");
				e.css({
					left : 0,
					top : 0,
					width : h.width(),
				...
				var f = a.top + this.target.outerHeight();
				b.css({
					left : 0,
					top : f,
					width : h.width(),
				...
				var g = a.left + this.target.outerWidth();
				d.css({
					left : g,
					top : a.top,
					width : h.width() - g,
				...
				e.appendTo(document.body).show();
				b.appendTo(document.body).show();
				i.appendTo(document.body).show();
				d.appendTo(document.body).show();
				this.blockEvents()
			},
...
PrimeFaces.widget.Sticky = PrimeFaces.widget.BaseWidget.extend({
	...
	fix : function() {
		if (!this.fixed) {
			...
			$('<div class="ui-sticky-ghost"></div>').height(
...
PrimeFaces.widget.TabView = PrimeFaces.widget.BaseWidget
		.extend({
			init : function(c) {
				this._super(c);
				...
				if (this.cfg.scrollable) {
					if (this.jq.is(":visible")) {
						...
						var a = this.jq.parent()
								.closest(".ui-hidden-container"), b = a
								.data("widget"), e = this;
						if (b) {
							var d = PF(b);
							if (d) {
								d.addOnshowHandler(this.id, function() {
									return e.initScrolling()
								})
							}
						}
					}
					...
					d
							.hide(
									...
									function() {
										...
										b
												.addClass("ui-state-focus ui-tabs-selected ui-state-active");
			...
			show : function() {
				if (this.target) {
					...
					this.timeout = setTimeout(function() {
						...
						a.jq.show(a.cfg.showEffect, {}, 250)
					}, this.cfg.showDelay)
				}
			},
			hide : function() {
		...
		.extend({
			...
			partialCheck : function(d) {
				...
				c.find("> .ui-treenode-content > .ui-treenode-label")
						.removeClass("ui-state-highlight");
				a.removeClass("ui-icon-blank fa fa-fw fa-check-square-o").addClass(
						"ui-icon-minus");
			...
			uncheck : function(d) {
				var b = d.children(".ui-chkbox-box"), a = b
						...
						.closest(".ui-treenode"), e = this.getRowKey(c);
				b.removeClass("ui-state-hover");
				a.removeClass("ui-icon-minus fa fa-fw fa-check-square-o").addClass(
						"ui-icon-blank");
...
PrimeFaces.widget.VerticalTree = PrimeFaces.widget.BaseTree
		.extend({
			...
			collapseNode : function(g) {
				...
				var f = g.find("> .ui-treenode-content > .ui-tree-toggler"), d = g
						.data("nodetype"), c = f.next(), a = this.cfg.iconStates[d], e = g
						.children(".ui-treenode-children");
				f.addClass(b.cfg.collapsedIcon).removeClass(
						"ui-icon-triangle-1-s");
				if (a) {
			...
			showNodeChildren : function(e) {
				...
				var d = e.find("> .ui-treenode-content > .ui-tree-toggler"), c = e
						.data("nodetype"), b = d.next(), a = this.cfg.iconStates[c];
				d.addClass("ui-icon-triangle-1-s").removeClass(
						this.cfg.collapsedIcon);
				if (a) {
			...
			preselectCheckbox : function() {
				this.jq
						.find(".ui-chkbox-icon")
						.not(".fa fa-fw fa-check-square-o")
						.each(
								function() {
									...
									if (b
											.children(".ui-treenode-children")
											.find(
													".ui-chkbox-icon.fa fa-fw fa-check-square-o").length > 0) {
...
PrimeFaces.widget.HorizontalTree = PrimeFaces.widget.BaseTree
		.extend({
			...
			showNodeChildren : function(e) {
				...
				if (a) {
					d.next().removeClass(a.collapsedIcon).addClass(
							a.expandedIcon)
				}
				d.addClass("ui-icon-minus").removeClass("fa fa-fw fa-plus-square-o");
			...
			collapseNode : function(e) {
				var c = e.next(), d = e
						...
						.data("nodetype"), a = this.cfg.iconStates[b];
				if (a) {
					d.next().addClass(a.collapsedIcon).removeClass(
							a.expandedIcon)
				}
				d.removeClass("ui-icon-minus").addClass("fa fa-fw fa-plus-square-o");
				...
				this.jq
						...
						.each(
								function() {
									...
									if (b.find(".ui-chkbox-icon.fa fa-fw fa-check-square-o").length > 0) {
				...
				if (this.isCheckboxSelection()) {
					if (this.cfg.nativeElements) {
						...
						c
								...
								.addClass("ui-icon-blank").removeClass(
										"fa fa-fw fa-check-square-o ui-icon-minus")
					}
				}
				if (!a) {
				...
				if (j) {
					...
					if (f) {
						...
						if (this.cfg.nativeElements) {
							...
							g.removeClass("ui-icon-blank fa fa-fw fa-check-square-o")
									.addClass("ui-icon-minus")
						}
			...
			setupScrolling : function() {
				...
				if (this.cfg.scrollHeight) {
					...
					if (this.hasVerticalOverflow()) {
						...
						this.scrollHeaderBox.css("margin-right", b);
						this.scrollBody.css("padding-right", b);
						this.scrollFooterBox.css("margin-right", b)
					}
				}
				this.fixColumnWidths();
