...
@FacesValidator("validateVLimit")
public class ValidateVersionLimit implements Validator, ClientValidator {
		...
		try {
			...
			if (versionLimit <= 0) {
				msg = new FacesMessage(MsgTitleEnum.ERROR.getMsg(),
						ViewError.LIMIT_VERSION_EQ_ZERO.getMsg());
				msg.setSeverity(FacesMessage.SEVERITY_ERROR);
				throw new ValidatorException(msg);
			}
		} catch (Exception e) {
			msg = new FacesMessage(MsgTitleEnum.ERROR.getMsg(),
					ViewError.LIMIT_VERSION_INVALID.getMsg());
			msg.setSeverity(FacesMessage.SEVERITY_ERROR);
			throw new ValidatorException(msg);
		}
	}
